    ...
    public static Connection getConnection () {
        if (instance == null) { // Connection 객체가 없다면
            try { // 객체 생성 작업
                ...
                // 연결할 데이터베이스의 정보
                String dbUrl = "jdbc:oracle:thin:@localhost:1521:orcl"; // 데이터베이스 URL(연결문자열
                String username = "school"; // 오라클 계정명(사용자명)
                String password = "1234"; // 오라클 계정 비밀번호

                // DriverManager 를 통해 Connection 객체를 생성
                // 매개변수를 연결할 데이터베이스의 URL, 사용자 이름, 비밀번호를 전달
